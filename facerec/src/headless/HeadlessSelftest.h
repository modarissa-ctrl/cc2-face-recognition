/**
 * @file HeadlessSelftest.h
 * @brief Headless self-test suite extracted from `main.cpp`.
 */
 ...
 */
inline void selftestTracker(const std::function<void(bool, const std::string &)> &check)
{
 ...
 */
inline void selftestLiveness(const std::function<void(bool, const std::string &)> &check)
{
    ...
    check(statuses.size() == 3 && statuses[0].verdict == LivenessDetector::Verdict::Live && statuses[0].blinkCount >= 2,
...
inline int runSelftest()
{
    ...
    detail::selftestTracker(check);
    detail::selftestLiveness(check);
